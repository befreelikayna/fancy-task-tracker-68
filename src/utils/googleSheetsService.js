...
async function getAccessToken() {
  try {
    ...
    // Since we can't use the google-auth-library in the browser,
    // we'll need to make the token request directly
    const response = await fetch('https://oauth2.googleapis.com/token', {
      ...
      body: new URLSearchParams({
        ...
        assertion: btoa(JSON.stringify(header)) + '.' + 
                  btoa(JSON.stringify(claim)) + '.' +
                  btoa(PRIVATE_KEY.split('\\n').join(''))
      })
    });
